...
export function GeomanEvents() {
  ...
  useEffect(() => {
    ...
    const handleEdit = (e: AnyEvent) => {
      const layer = e.layer;
      // The layer instance is passed by reference, so we can find its ID
      const polygonId = (layer as AnyLayer)._polygonId;
      ...
      if (polygonId) {
        ...
        if (!validation.isValid) {
          alert(
            validation.error || "Invalid polygon shape after edit. Reverting."
          );
